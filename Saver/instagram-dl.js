...
(function () {
    ...
    var svgDownloadBtn = `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" height="24" width="24" viewBox="0 0 477.867 477.867" xml:space="preserve">
    <defs>
        <linearGradient id="iconGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="#F09C30" />
            <stop offset="100%" stop-color="#F52A3A" />
        </linearGradient>
    </defs>
    <circle cx="50%" cy="50%" r="50%" fill="url(#iconGradient)" />
     <g style="fill: #ffffff; stroke: #ffffff; stroke-width: 13;">
        <!-- Center the white icon with scale(0.5) -->
        <g transform="translate(70, 40) scale(0.7)" text-anchor="middle" dominant-baseline="central">
            <!-- Add the first path -->
    ...
    function addCustomBtn(node, iconColor, appendNode) {  //hiten
        let newtabBtn = createCustomBtn(svgDownloadBtn, iconColor, 'newtab-btn', '16px');
    ...
    function openResource(url) { //hiten
        // open url in new tab
//        console.log(`Dowloading 123 ${url}`);
        window.webkit.messageHandlers.buttonClicked.postMessage(url);
    }
    ...
    function downloadResource(url, filename) { ////hiten
        if (url.startsWith('blob:')) {
            forceDownload(url, filename, 'mp4');
            return;
        }
//        console.log(`Dowloading 456 ${url}`);
        window.webkit.messageHandlers.buttonClicked.postMessage(url);
